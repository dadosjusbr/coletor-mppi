    ...
    for row in rows:
        ...
        if not parser.isNaN(matricula):
            if "Membros" not in str(matricula) and "Matrícula" not in matricula:
                ...
                imp_renda = parser.format_value(row[17])  # Imposto de Renda
                teto_constitucional = parser.format_value(
                    row[18]
                )  # Retenção por Teto Constitucional
                indenizacoes = parser.format_value(row[21])
                outras_remuneracoes_temporarias = parser.format_value(row[22])
                    ...
                    "income": {
                        ...
                        "other": {  # Gratificações
                            ...
                            "others": {
                                ...
                                "Outra Remunerações Temporárias": outras_remuneracoes_temporarias,
                            },
                        },
                    },
